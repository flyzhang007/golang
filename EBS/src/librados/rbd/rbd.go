...
// #cgo LDFLAGS: -L${SRCDIR}/../libs/ -lrbd -lrados
// #include <errno.h>
// #include <stdlib.h>
...
var RbdErrorAlreadyExist = errors.New("RBD image already exist")
var RbdErrorProtected = errors.New("Device or resource busy")
...
func GetError(err C.int) error {
	if err != 0 {
		...
		if err == -C.EEXIST {
			return RbdErrorAlreadyExist
		}
		if err == -C.EBUSY {
			return RbdErrorProtected
		}
		return RBDError(err)
	} else {
		return nil
...
func (image *Image) GetStripePeriod() (stripe_period uint64, err error) {
	if image.image == nil {
		return 0, RbdErrorImageNotOpen
	}

	var stripe_count uint64
	...
	var c_stat C.rbd_image_info_t
	if ret := C.rbd_stat(image.image, &c_stat, C.size_t(unsafe.Sizeof(c_stat))); ret < 0 {
		return 0, RBDError(int(ret))
	}

	stripe_period = stripe_count * (1 << uint64(c_stat.order))

	return stripe_period, nil
}
...
func (image *Image) Read2(data []byte, op_flag int) (n int, err error) {
	if image.image == nil {
		return 0, RbdErrorImageNotOpen
	}
	...
	ret := int(C.rbd_read2(
		...
		(C.uint64_t)(image.offset),
		(C.size_t)(len(data)),
		(*C.char)(unsafe.Pointer(&data[0])), (C.int)(op_flag)))
...
func (image *Image) Read2At(data []byte, off int64, op_flag int) (n int, err error) {
	if image.image == nil {
		return 0, RbdErrorImageNotOpen
	}
